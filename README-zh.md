...
**最新加固（v6.3.3+）：** 每次 **Periodic Hard Reset（周期硬重置）** 后，报价引擎 **强制** 调用 Polymarket **Data API** 持仓对账；若对账 **失败**，**当前 tick 冻结全部新买单**（仍允许 **SELL/减仓**）。单市场敞口采用 **严格盯市（MTM）持仓估值 + 全市场活跃 BUY 挂单名义** 口径；后台 REST 对账默认 **每 60 秒**（`RECONCILIATION_INTERVAL_SEC`）。
...
- **幽灵订单硬重置 + 强制 REST 对账** — 约 **每 5 分钟** **Periodic Hard Reset** 清理 WS 幽灵单；撤单失败则 **force_evict** 本地活动单缓存。重置后 **立即** 调用 **`watchdog.reconcile_single_market(condition_id, force=True)`** 对齐 Data API，再重建网格。若对账 **失败**，本 tick **不挂新 BUY**，预检前剔除 BUY（**半休克**），并发布模式 **`POST_RESET_RECONCILE_FREEZE`**。

...
| **风控** | 跨全市场强制生效的全局预算红线 (`GLOBAL_MAX_BUDGET`)。Kill Switch、熔断器、单市场敞口上限、带时间保护的对账、下单前余额预检。 |
...
- **全局余额预检** — 下单前不仅检查 `MAX_EXPOSURE_PER_MARKET`，还会向内存索要全局敞口验证是否越过 `GLOBAL_MAX_BUDGET`。超预算自动缩档或砍档，彻底防爆仓。
...
- **幽灵订单硬重置** — 周期性 TTL hard reset 清理 WS 掉线导致的 phantom orders；硬重置时若撤单失败会强制从本地活动订单缓存剔除，以释放 pending notional 预算。
- **OMS + 熔断器** — 通过 `py-clob-client` 下单/撤单；瞬时错误触发熔断；非瞬时（如 400）不触发；「已成交订单无法撤」按成功处理（INFO）。
- **风控守护（Watchdog）** — 单市场 kill switch 以**已花出去的 capital_used**为准（不将 pending notional 作为硬停依据）；定期与 Polymarket 持仓 API 对账，带容差、时间保护，并同步/修正 capital_used 防止“幽灵预算锁死”。

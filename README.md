...
> **Deployment status: locked by default.** The live execution code path has completed offline engineering remediation, but this repository cannot certify a funded deployment without external evidence. Historical ledger rebuild, hash-pinned out-of-sample alpha evidence, wallet funding/allowance and deployment connectivity checks remain operator prerequisites. No exchange connection or live order was used for this review.
...
- A Postgres wallet lease prevents concurrent live executors and is renewed immediately before every submit. Sticky halts survive restarts and require an explicit disabled/flat acknowledgement; stream or geographic uncertainty halts new risk.
...
- Every processed fill has a one-to-one immutable signed cash fact and inventory state version. Makers are fee-free; taker fees are calculated from authenticated `fee_rate_bps` using the documented five-decimal USDC formula. Missing fee inputs remain `UNKNOWN`, invalidate net PnL and halt new live risk.
...
- The staged operator checklist, arm sequence, release decision and incident rules are in the [funded deployment runbook](spec/LIVE_DEPLOYMENT_RUNBOOK.md).
...
- **Fail-closed reconciliation.** In live mode, authenticated CLOB conditional-token balances are authoritative for known and active tokens; the public **Data API** is only used to discover unknown positions and provide cost metadata when sizes agree. Any size overwrite invalidates exact cost/fee/PnL accounting and activates a sticky halt; it is never presented as a successful accounting repair. The legacy Periodic Hard Reset path has been deleted.
...
Do not arm a funded deployment until every status/readiness gate passes and profitability is demonstrated through offline replay, fee attribution and markout analysis. Passing engineering checks does not imply profitable trading.
...
| **Execution** | Diff quoting (keep/cancel/create by signature); preserves time priority and cuts API churn. Native SDK streams fail closed on disconnect, reconnect gaps and bounded-queue drops. |
...
│ • quote risk gates  │ auth bal. │ • periodic compare      │
...
| [Getting started](https://docs.polymarket.com/trading/quickstart) | Fund collateral, configure the wallet and approve tokens | The pinned unified SDK creates/derives authenticated credentials from `PK` and verifies its wallet against `FUNDER_ADDRESS`. Funding and approvals remain explicit operator steps outside the app. |
...
| `app/oms/core.py` | OMS: unified V2 adapter, new-order CircuitBreaker, priority cancellation, per-submit lease renewal, stable local/exchange IDs and conservative unknown outcomes. |
| `app/risk/watchdog.py` | **~1s** cold/hot cost + reservation checks; authenticated token-balance reconciliation with Data API discovery; mismatch invalidates accounting; kill switch → halt + cancel + suspend. |
...
- Polymarket account and the current V2 collateral asset (pUSD)
...
For first deployment, keep `TRADING_MODE=disabled`. This starts only the local control plane and dependencies while migrations, accounting evidence and configuration are inspected.
...
| `EXECUTION_LEASE_TTL_SEC` / `ORDER_RECONCILIATION_INTERVAL_SEC` | Single-wallet writer lease / authenticated order, trade and position reconciliation | `15` / `60` |
...
| `RECONCILIATION_INTERVAL_SEC` | Watchdog authenticated token-balance reconciliation interval | `300` |

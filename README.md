...
- **Dashboard（Streamlit 驾驶舱）**
  - **Control Panel：**
    - 支持直接输入 **Condition ID 或 Polymarket URL**（例如 `https://polymarket.com/event/<slug>`），一键调用 `/markets/{condition_id}/start`，自动：
      - 从 Gamma 获取 CLOB tokenIds；
      - 订阅 Market WS + User WS；
      - 启动双 token 的 QuotingEngine；
      - 触发 REST 初始快照 + 第一轮网格挂单。
    - 提供显式确认勾选框（`I understand this may place real orders with current config`），提示“当前配置可能会真实下单”，避免误触。
  - **Emergency Controls：**
    - 输入 `condition_id` 后：
  ...
  - **Market Explorer (Gamma)：**
    - 一键从 Gamma API 拉取一批当前 **活跃的热门市场**（`active=true, closed=false`）。
    - 每个市场卡片展示 question、conditionId、slug、24h 成交量与流动性，并提供直达 Polymarket 的链接。
    - 支持在卡片上直接点击「Confirm & Start Quoting」按当前 `.env` 配置启动该市场做市，无需手动复制 conditionId。

...
```mermaid
flowchart TB
    A["静态授权<br/>mode + wallet allowlist + expiry + budget + build identity"]
...
- live 已知/活跃 token 由认证 CLOB 余额核对；Data API 只用于发现未知仓位与可核对的成本元数据。任何覆盖后账本标记 `unverified_external`，不能继续当作精确会计。
